<script lang="ts">
	...
	function setup() {
		...
		canvasWidth = Math.min(parseInt(/^([\s\S]+)px$/.exec(widthStr)![1]) - 48, 600);
		render();
	...
	function render(updateSelectedArea: boolean = false) {
		...
		for (let [color, name, weight] of creditTable) {
			...
			let percent = (100 * credits[name] * weight / totalCredit).toFixed(2) + '%';
			ctx.beginPath();
			ctx.moveTo(xy, xy);
			...
			if (simplifyA(midA) > Math.PI / 2) {
				...
				let x = lineEndX + (canvasWidth / 10);
				ctx.lineTo(x, lineEndY);
				ctx.fillText(name + credits[name], lineEndX, lineEndY - canvasWidth / 120);
...
<Section>
	...
	<div class="parent">
		<a style="font-size: 24px; color: black;" href={'https://www.mcbbs.net/home.php?mod=space&username=' + renderingUser.username} target="_blank">
		...
		{#each creditTable as credit}
			<div class="lines" style={selectedArea === credit[1] ? "background: #d8d8d8;" : ""} on:click={() => selectArea(credit[1])}>
				...
				<div class="percent">
					{(100 * credits[credit[1]] * credit[2] / totalCredit).toFixed(2)}%
				</div>
				<div class="rank">
